...
SafeGuard 后端采用全新的无耦合数据库架构，实现了数据库与 Flask 框架的完全解耦。本文档提供完整的使用指南。

## 架构特点

-   **完全解耦**：数据库层不依赖 Flask 框架
-   **模式灵活**：支持三种使用模式
...
4. **性能优化**：批量操作使用 bulk_save_mappings
5. **安全性**：生产环境使用连接池和 SSL
...
from database.flask_models import NewTable
...
def test_new_table_with_existing_data():
    ...
    with db.get_session() as session:
        from database.flask_models import User, NewTable
...
2. 生成 Alembic 迁移：`alembic revision --autogenerate -m "描述"`
3. 编写单元测试验证功能
4. 在测试环境验证迁移
5. 生产环境执行迁移
