...
class CommunityEvent(Base, QueryMixin):
    """社区事件表"""
    ...
    event_type = Column(String(50), nullable=False, default='call_for_help', comment='事件类型')
    status = Column(Integer, default=1, comment='事件状态：1-进行中，2-已完成，3-已取消')
    target_user_id = Column(Integer, ForeignKey('users.user_id'), comment='目标用户ID')
    location = Column(String(200), comment='事件地点')
    created_by = Column(Integer, ForeignKey('users.user_id'), nullable=False, comment='创建者ID')
    created_at = Column(DateTime, default=datetime.now)
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now)
    ...
    target_user = relationship('User', foreign_keys=[target_user_id], backref='targeted_events')
    supports = relationship('EventSupport', backref='event', cascade='all, delete-orphan')
...
class EventSupport(Base, QueryMixin):
    """事件应援记录表"""
    __tablename__ = 'event_supports'

    support_id = Column(Integer, primary_key=True, autoincrement=True)
    event_id = Column(Integer, ForeignKey('community_events.event_id'), nullable=False)
    supporter_id = Column(Integer, ForeignKey('users.user_id'), nullable=False)
    support_content = Column(Text, comment='应援内容')
    status = Column(Integer, default=1, comment='应援状态：1-有效，2-已取消')
    created_at = Column(DateTime, default=datetime.now)
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now)

    # 关系
    supporter = relationship('User', backref='supports')
    ...
    # 唯一约束：同一事件同一用户只能应援一次
    __table_args__ = (
        UniqueConstraint('event_id', 'supporter_id', name='uq_event_supporter'),
    )
...
__all__ = [
    ...
    'UserCommunityRule', 'Counters', 'CommunityEvent', 'EventSupport'
]