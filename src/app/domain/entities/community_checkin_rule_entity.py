...
class CommunityCheckinRuleEntity:
    ...
    def __init__(self, rule: CommunityCheckinRule):
        """
        初始化社区打卡规则领域实体

        Args:
            rule: SQLAlchemy CommunityCheckinRule 模型实例
        """
        self._rule = rule

    @property
    def rule(self) -> CommunityCheckinRule:
        """获取底层的 SQLAlchemy CommunityCheckinRule 模型"""
        return self._rule

    @property
    def rule_id(self) -> int:
        """获取规则ID"""
        return self._rule.community_rule_id

    @property
    def community_id(self) -> int:
        """获取社区ID"""
        return self._rule.community_id
    ...
    def __eq__(self, other) -> bool:
        if not isinstance(other, CommunityCheckinRuleEntity):
            return False
        return self._rule.community_rule_id == other._rule.community_rule_id

    def __hash__(self) -> int:
        return hash(self._rule.community_rule_id)