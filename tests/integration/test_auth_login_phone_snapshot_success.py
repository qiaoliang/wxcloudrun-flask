...
class TestAuthLoginPhoneSnapshotSuccess:
    """成功的手机号登录API快照对比测试类"""
    ...
    def test_login_phone_performance_consistency(self):
        ...
        if len(set(tokens)) < len(tokens):
            print(f"⚠️  注意: 某些token可能相同（时间戳精度问题），这是正常的")
        else:
            print(f"✅ Token唯一性验证通过：{len(tokens)} 个token均唯一")
        
        if len(set(refresh_tokens)) < len(refresh_tokens):
            print(f"⚠️  注意: 某些refresh_token可能相同，这是正常的")
        else:
            print(f"✅ Refresh Token唯一性验证通过：{len(refresh_tokens)} 个token均唯一")
    ...
    def test_login_phone_business_logic_consistency(self):
        """测试登录API的业务逻辑一致性"""
        client = self.get_test_client()
        ...
        business_logic_checks = [
            (response_data['user_id'] == self.test_user.user_id, "用户ID应该匹配"),
            (response_data['phone_number'] == self.test_user.phone_number, "手机号应该匹配"),
            (response_data['wechat_openid'] == self.test_user.wechat_openid, "微信OpenID应该匹配"),
            (response_data['nickname'] == self.test_user.nickname, "昵称应该匹配"),
            (response_data['role'] == '普通用户', "角色名称应该正确映射"),
            (response_data['community_id'] == self.test_community.community_id, "社区ID应该匹配"),
            (response_data['community_name'] == '测试社区', "社区名称应该匹配"),
            (response_data['login_type'] == 'existing_user', "登录类型应该为existing_user"),
            (response_data['avatar_url'] == self.test_user.avatar_url, "头像URL应该匹配"),
            (response_data['name'] == self.test_user.name, "姓名应该匹配")
        ]
        
        failed_checks = [check for check, msg in business_logic_checks if not check]
        
        assert not failed_checks, f"业务逻辑一致性验证失败: {failed_checks}"
        print(f"✅ 业务逻辑一致性验证通过：{len(business_logic_checks)} 项检查通过")
