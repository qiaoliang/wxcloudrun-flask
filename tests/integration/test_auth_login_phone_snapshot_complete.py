...
class TestAuthLoginPhoneSnapshotComplete:
    """完整的手机号登录API快照对比测试类"""
    ...
    def _create_test_data(cls):
        ...
        # 创建测试用户
        cls.test_user = User(
            wechat_openid='test_snapshot_user',
        ...
        # 保存预期值用于快照对比（避免SQLAlchemy session问题）
        cls.expected_values = {
    ...
    def test_login_phone_multiple_roles_consistency(self):
        """测试多种用户角色的数据一致性"""
        role_test_cases = [
            {'role': 2, 'expected_role_name': '社区专员', 'nickname': '社区专员测试', 'phone': '13900007998'},
            {'role': 3, 'expected_role_name': '社区主管', 'nickname': '社区主管测试', 'phone': '13900007999'},
            {'role': 4, 'expected_role_name': '超级系统管理员', 'nickname': '超级管理员测试', 'phone': '13900008000'}
        ]
        
        for test_case in role_test_cases:
            print(f"\n🧪 测试角色: {test_case['expected_role_name']}")
            
            # 创建测试用户
            from hashlib import sha256
            phone_secret = os.getenv('PHONE_ENC_SECRET', 'default_secret')
            phone_hash = sha256(f"{phone_secret}:{test_case['phone']}".encode('utf-8')).hexdigest()
            
            test_user = User(
                wechat_openid=f'test_role_user_{test_case["role"]}',
                ...
                status=1,
                password_salt='test_salt',
                password_hash=sha256(f"Firefox0820:test_salt".encode('utf-8')).hexdigest(),
            ...
            try:
                ...
                # 验证角色一致性
                assert response_data['role'] == test_case['expected_role_name']
                assert response_data['nickname'] == test_case['nickname']
                assert response_data['phone_number'] == test_case['phone']
                assert response_data['community_name'] == '测试社区'
                assert response_data['login_type'] == 'existing_user'
                
                print(f"✅ 角色 {test_case['expected_role_name']} 数据一致性验证通过")
