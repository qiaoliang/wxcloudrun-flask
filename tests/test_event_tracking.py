...
from database.flask_models import db, User, Community, CommunityEvent, EventSupport, EventClosure
...
class TestEventModels:
    ...
    def test_event_support_model_fields(self, setup_app):
        """测试 EventSupport 模型字段"""
        with setup_app.app_context():
            # 检查 EventSupport 模型是否有新增字段
            columns = [column.name for column in EventSupport.__table__.columns]
            
            assert 'message_type' in columns
            assert 'media_url' in columns
            assert 'media_duration' in columns
            assert 'support_tags' in columns
    ...
    def test_event_support_to_dict(self, setup_app):
        """测试 EventSupport to_dict 方法"""
        with setup_app.app_context():
            # 创建测试数据
            community = Community(
                name='测试社区',
            ...
            support = EventSupport(
                ...
                supporter_id=user.user_id,
                support_content='测试内容',
                message_type='text',
                media_url='http://example.com/media.mp3',
                media_duration=30,
                support_tags=['标签1', '标签2']
            )
            ...
            assert data['support_tags'] == ['标签1', '标签2']
