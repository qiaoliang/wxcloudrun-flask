...
from app.application.use_cases.supervision.invite_supervisor_use_case import InviteSupervisorUseCase
...
class TestInviteSupervisorUseCase:
    """测试InviteSupervisorUseCase"""

    @patch('app.application.use_cases.supervision.invite_supervisor_use_case.RepositoryFactory')
    def test_should_successfully_invite_supervisor_with_specific_rules(self, mock_repo_factory):
        """应该成功邀请监督者监督特定规则"""
        # Arrange - 准备测试数据和Mock
        mock_user_repo = Mock()
        mock_rule_repo = Mock()
        ...
        mock_user_repo.find_by_id.side_effect = lambda id: inviter if id == 1 else target_user
        ...
        saved_relation.rule_id = 101
        saved_relation.supervisor_user_id = 1
        saved_relation.solo_user_id = 2
        ...
        assert result.data['relations'][0]['rule_id'] == 101
        mock_relation_repo.save.assert_called_once()

    @patch('app.application.use_cases.supervision.invite_supervisor_use_case.RepositoryFactory')
    def test_should_fail_when_inviter_not_found(self, mock_repo_factory):
        """应该在邀请者不存在时失败"""
        # Arrange
        mock_user_repo = Mock()
        mock_repo_factory.get_user_repository.return_value = mock_user_repo
        ...
        assert '邀请者不存在' in result.message

    @patch('app.application.use_cases.supervision.invite_supervisor_use_case.RepositoryFactory')
    def test_should_fail_when_target_user_not_found(self, mock_repo_factory):
        """应该在目标用户不存在时失败"""
        # Arrange
        mock_user_repo = Mock()
        mock_repo_factory.get_user_repository.return_value = mock_user_repo
        ...
        inviter = Mock()
        inviter.user_id = 1
        mock_user_repo.find_by_id.side_effect = lambda id: inviter if id == 1 else None
    ...
    def test_should_fail_when_rule_not_found(self, mock_repo_factory):
        ...
        assert not result.is_success
        assert result.status == UseCaseStatus.NOT_FOUND
        assert '规则ID 999 不存在' in result.message

    @patch('app.application.use_cases.supervision.invite_supervisor_use_case.RepositoryFactory')
    def test_should_fail_when_no_permission_for_rule(self, mock_repo_factory):
        """应该在无权限操作规则时失败"""
        # Arrange
        mock_user_repo = Mock()
        mock_rule_repo = Mock()
        ...
        assert '无权限操作规则ID 101' in result.message
