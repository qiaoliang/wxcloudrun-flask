...
class TestUserCommunityRuleSwitching:
    ...
    @pytest.fixture
    def setup_test_data(self, test_app):
        """设置测试数据"""
        import time
        import uuid

        # 使用 Flask-SQLAlchemy 的会话
        ...
        with test_app.app_context():
            ...
            db.session.add_all([community_a_rule1, community_a_rule2])
            ...
            db.session.add_all([community_b_rule1, community_b_rule2])
    ...
    def test_initial_community_assignment(self, setup_test_data, test_app):
        ...
        # 验证数据库状态
        from database.flask_models import db
        active_mappings = db.session.query(UserCommunityRule).filter_by(
            user_id=user_id,
            is_active=True
        ).all()
    ...
    def test_community_switching(self, setup_test_data, test_app):
        ...
        # 检查社区B规则状态
        b_mappings = db.session.query(UserCommunityRule).join(CommunityCheckinRule).filter(
