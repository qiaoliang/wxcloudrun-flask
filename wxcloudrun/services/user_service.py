...
class CheckinRuleService(BaseService):
    ...
    def create_rule(self, user_id: int, rule_data: Dict[str, Any]) -> CheckinRule:
        ...
        new_rule = CheckinRuleModel(
            ...
            custom_time=dt.strptime(rule_data['custom_time'], '%H:%M').time() if rule_data.get('custom_time') else None,
    ...
    def update_rule(self, rule: CheckinRule, rule_data: Dict[str, Any]) -> CheckinRule:
        ...
        if 'custom_time' in rule_data:
            from datetime import datetime as dt
            rule.custom_time = dt.strptime(rule_data['custom_time'], '%H:%M').time() if rule_data['custom_time'] else None
...
class CheckinRecordService(BaseService):
    ...
    def perform_checkin(self, rule_id: int, user_id: int) -> Dict[str, Any]:
        ...
        return {
            'rule_id': rule_id,
            'record_id': record_id,
            'checkin_time': checkin_time.strftime('%Y-%m-%d %H:%M'),
            'message': '打卡成功'
        }
    ...
    def get_checkin_history(self, user_id: int, start_date: date, end_date: date) -> List[Dict[str, Any]]:
        ...
        for record in records:
            history_data.append({
                ...
                'planned_time': record.planned_time.strftime('%Y-%m-%d %H:%M'),
                'checkin_time': record.checkin_time.strftime('%Y-%m-%d %H:%M') if record.checkin_time else None,
